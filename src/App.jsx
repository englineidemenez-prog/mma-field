...
export default function App() {
  ...
  const lerMTR = async arq => {
    ...
    try {
      ...
      var pt = "Analise MTR ou CDF. Retorne APENAS JSON: tipo_doc, numero, data_emissao, classe, tipo_residuo, volume, unidade, transportador, destinador, tratamento";
      ...
      var rs = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,messages:[{role:"user",content:mc}]})});
      var j = await rs.json();
  ...
  return (
    <div style={{minHeight:"100vh",background:"#eef1ee",fontFamily:"Georgia,serif"}}>
      ...
      <main style={{maxWidth:1100,margin:"0 auto",padding:"20px"}}>
              ...
              <div style={CD}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:8}}>
                  <label style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,padding:"14px 8px",border:"2px dashed #a8c5b5",borderRadius:10,cursor:"pointer",background:"#f8fdf9"}}>
                    ...
                    <span style={{fontSize:11,color:"#2d6a4f",fontWeight:"bold"}}>Câmera</span>
                    <span style={{fontSize:10,color:"#888"}}>Com GPS automático</span>
                    <input ref={ref} type="file" accept="image/*" capture="environment" onChange={onFoto} style={{display:"none"}}/>
                  </label>
                  <label style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,padding:"14px 8px",border:"2px dashed #a8c5b5",borderRadius:10,cursor:"pointer",background:"#f8fdf9"}}>
                    <span style={{fontSize:22}}>🖼️</span>
                    <span style={{fontSize:11,color:"#2d6a4f",fontWeight:"bold"}}>Galeria</span>
                    <span style={{fontSize:10,color:"#888"}}>Selecionar arquivo</span>
                ...
                <button onClick={addF} disabled={!prev||!psel} style={{background:!prev||!psel?"#ccc":"linear-gradient(135deg,#2d6a4f,#1a3d2b)",color:"#fff",border:"none",borderRadius:8,padding:"10px",width:"100%",fontSize:13,cursor:!prev||!psel?"not-allowed":"pointer",fontFamily:"Georgia,serif",fontWeight:"bold"}}>
                  ✓ Adicionar Foto
                </button>
              </div>
              <div>
                ...
                <div>
                      ...
                      <div style={CD}>
                        ...
                        <div style={{overflowX:"auto"}}>
                          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:860}}>
                            <thead><tr>{["Doc","Número","Data","Classe","Tipo","Volume","Transportador","Destinador","Tratamento",""].map((h,i)=><th key={i} style={{...TH,background:"#5a4fcf",padding:"6px 7px",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
                            <tbody>
                              {inv.length===0&&<tr><td colSpan={10} style={{...TD,textAlign:"center",color:"#ccc",padding:"20px",fontStyle:"italic"}}>Nenhum registro — faça upload de MTR ou CDF acima</td></tr>}
                              {inv.map((row,i)=>(
                                <tr key={row.id}>
                                  {row.ed?(
                                    <>{["tipo_doc","numero","data","classe","tipo_residuo","volume","transportador","destinador","tratamento"].map(f=>(
                                    ...
                                    <>{[row.tipo_doc,row.numero,row.data,row.classe,row.tipo_residuo,(row.volume||"")+" "+(row.unidade||""),row.transportador,row.destinador,row.tratamento].map((v,ci)=>(
                                      <td key={ci} style={{...(i%2?TA:TD),whiteSpace:"nowrap",maxWidth:140,overflow:"hidden",textOverflow:"ellipsis"}}>{v||"—"}</td>
                                    ))}
                        ...
                        return (
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                            <div style={CD}>
                              ...
                              <div style={{fontSize:10,color:"#888",marginBottom:8}}>Total: {tG.toFixed(2)} t</div>
                              <ResponsiveContainer width="100%" height={200}><PieChart><Pie data={dp} cx="50%" cy="50%" innerRadius={40} outerRadius={80} dataKey="value" paddingAngle={2}>{dp.map((_,i)=><Cell key={i} fill={COR[i%COR.length]}/>)}</Pie><Tooltip formatter={(v,n,pr)=>[v+" t ("+pr.payload.pct+"%)",n]}/></PieChart></ResponsiveContainer>
                              <div style={{marginTop:6,display:"flex",flexDirection:"column",gap:3}}>{dp.map((d,i)=>(<div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:10}}><div style={{display:"flex",alignItems:"center",gap:5}}><div style={{width:10,height:10,borderRadius:2,background:COR[i%COR.length],flexShrink:0}}/><span>{d.name}</span></div><div style={{display:"flex",gap:8}}><span style={{color:"#888"}}>{d.value} t</span><span style={{fontWeight:"bold",color:COR[i%COR.length],minWidth:36,textAlign:"right"}}>{d.pct}%</span></div></div>))}</div>
                            </div>
                            <div style={CD}>
                              <div style={{fontSize:12,fontWeight:"bold",color:"#5a4fcf",marginBottom:2}}>📊 Geração Mensal (t)</div>
                              <div style={{fontSize:10,color:"#888",marginBottom:8}}>Volume por competência</div>
                              <ResponsiveContainer width="100%" height={200}><BarChart data={dm} margin={{top:5,right:10,left:0,bottom:dm.length>4?40:10}}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false}/><XAxis dataKey="mes" tick={{fontSize:9}} angle={dm.length>4?-35:0} textAnchor={dm.length>4?"end":"middle"} interval={0}/><YAxis tick={{fontSize:9}} unit=" t"/><Tooltip formatter={v=>[v+" t","Volume"]}/><Bar dataKey="vol" fill="#5a4fcf" radius={[4,4,0,0]} maxBarSize={48}/></BarChart></ResponsiveContainer>
                              {pico&&<div style={{marginTop:8,padding:"7px 10px",background:"#f8f7ff",borderRadius:7,border:"1px solid #e0dcff"}}><div style={{display:"flex",justifyContent:"space-between",fontSize:10}}><span style={{color:"#888"}}>Total acumulado</span><span style={{fontWeight:"bold",color:"#5a4fcf"}}>{dm.reduce((a,b)=>a+b.vol,0).toFixed(2)} t</span></div><div style={{display:"flex",justifyContent:"space-between",fontSize:10,marginTop:3}}><span style={{color:"#888"}}>Pico</span><span style={{fontWeight:"bold",color:"#5a4fcf"}}>{pico.mes} ({pico.vol.toFixed(2)} t)</span></div></div>}
                            </div>
                          </div>
                        );
                      })()}
                  ...
                  {pd==="supveg"&&(
                    <div style={CD}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                        <div>
                          <h3 style={{color:p.cor,fontSize:13,margin:0}}>🪓 Laudo de Cubagem</h3>
                          <div style={{fontSize:10,color:"#888",marginTop:2}}>Inventário florestal para cálculo de volume de madeira</div>
                        </div>
                  ...
                  <div style={CD}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                      ...
                      <button onClick={()=>{var g={id:Date.now(),titulo:"Novo Gráfico",tipo:"barra",cor:p.cor,unidade:"",dados:[]};set({graficos:[...(d.graficos||[]),g]});}} style={{background:p.cor,color:"#fff",border:"none",borderRadius:7,padding:"6px 14px",cursor:"pointer",fontSize:12,fontWeight:"bold"}}>+ Adicionar Gráfico</button>
                    </div>
                    ...
                    {(d.graficos||[]).map((gr,gi)=>{
                      ...
                      var addDado=()=>{var arr=[...(d.graficos||[])];arr[gi]={...arr[gi],dados:[...(arr[gi].dados||[]),{l:"",v:""}]};set({graficos:arr});};
                      var remDado=di=>{var arr=[...(d.graficos||[])];arr[gi]={...arr[gi],dados:(arr[gi].dados||[]).filter((_,j)=>j!==di)};set({graficos:arr});};
                      return(
                        <div key={gr.id} style={{border:"2px solid "+p.cor+"33",borderRadius:10,padding:14,marginBottom:14,background:"#fafdfb"}}>
                          ...
                          <div style={{marginBottom:8}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                              <label style={LB}>Rótulos e Valores</label>
                            ...
                            {(gr.dados||[]).length===0&&<div style={{fontSize:11,color:"#bbb",fontStyle:"italic"}}>Clique em "+ Adicionar Rótulo" para inserir dados.</div>}
                          </div>
                          {gd2.length>0&&(
                            <div style={{marginTop:8,padding:10,background:"#fff",borderRadius:8,border:"1px solid #e8e8e8"}}>
                              <ResponsiveContainer width="100%" height={180}>
                                {gr.tipo==="pizza"
                                  ?<PieChart><Pie data={gd2.map(g=>({name:g.l,value:Number(g.v)}))} cx="50%" cy="50%" outerRadius={65} dataKey="value" label={({name,percent})=>name+" "+(percent*100).toFixed(0)+"%"}>{gd2.map((_,i)=><Cell key={i} fill={COR[i%COR.length]}/>)}</Pie><Tooltip/></PieChart>
                                  :<BarChart data={gd2.map(g=>({name:g.l,val:Number(g.v)}))}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name" tick={{fontSize:9}}/><YAxis tick={{fontSize:9}} unit={gr.unidade?" "+gr.unidade:""}/><Tooltip formatter={v=>[v+(gr.unidade?" "+gr.unidade:""),"Valor"]}/><Bar dataKey="val" fill={gr.cor||p.cor} radius={[3,3,0,0]}/></BarChart>}
                              </ResponsiveContainer>
                            </div>
                          )}
                        </div>
            ...
            <div id="reldoc" style={{background:"#fff",borderRadius:14,boxShadow:"0 3px 20px rgba(0,0,0,0.10)",overflow:"hidden",border:"1px solid #dde5db"}}>
              ...
              <div style={{padding:"28px 40px"}}>
                ...
                <div style={{marginBottom:20}}>
                  <h2 style={{color:cor,fontSize:13,borderBottom:"2px solid "+cor,paddingBottom:5,marginBottom:10}}>1. PROGRAMAS EM EXECUÇÃO</h2>
                ...
                <h2 style={{color:cor,fontSize:13,borderBottom:"2px solid "+cor,paddingBottom:5,marginBottom:18}}>3. GESTÃO E SUPERVISÃO DOS PROGRAMAS AMBIENTAIS</h2>
                {ativos.map((p,pi)=>{
                  var d=getD(p.id);var fp=getF(p.id);
                  return(
                    <div key={p.id} style={{marginBottom:32}}>
                      <h3 style={{color:cor,fontSize:13,borderLeft:"4px solid "+p.cor,paddingLeft:10,marginBottom:10}}>{pi+1}. {getL(p.id).toUpperCase()}</h3>
                      {d.desc&&<p style={{fontSize:12,color:"#444",lineHeight:1.8,marginBottom:12,textAlign:"justify"}}>{d.desc}</p>}
                      {fp.length>0&&<div style={{marginBottom:12}}><h4 style={{fontSize:11,color:"#333",marginBottom:7}}>{pi+1}.1 Registro Fotográfico</h4><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>{fp.map((f,fi)=><div key={f.id} style={{border:"1px solid #ddd",borderRadius:8,overflow:"hidden"}}><img src={f.src} alt={f.leg} style={{width:"100%",height:140,objectFit:"cover",display:"block"}}/><div style={{padding:"5px 9px",background:"#fafafa",fontSize:10,textAlign:"center"}}>{f.geo&&<div style={{fontSize:8,color:"#888"}}>📍 {f.geo}</div>}<div>Foto {fi+1}{f.leg?" – "+f.leg:""}</div></div></div>)}</div></div>}
                      {(d.graficos||[]).filter(gr=>(gr.dados||[]).some(x=>x.l&&x.v)).map((gr,gi)=>{
                        var gd2=(gr.dados||[]).filter(x=>x.l&&x.v);
                        return gd2.length>0?(
                          <div key={gi} style={{marginBottom:12}}>
                            <h4 style={{fontSize:11,color:"#333",marginBottom:5}}>{gr.titulo||"Gráfico "+(gi+1)}</h4>
                            <ResponsiveContainer width="100%" height={160}>
                              {gr.tipo==="pizza"?<PieChart><Pie data={gd2.map(g=>({name:g.l,value:Number(g.v)}))} cx="50%" cy="50%" outerRadius={55} dataKey="value" label={({name,percent})=>name+" "+(percent*100).toFixed(0)+"%"}>{gd2.map((_,i)=><Cell key={i} fill={COR[i%COR.length]}/>)}</Pie><Tooltip/></PieChart>:<BarChart data={gd2.map(g=>({name:g.l,val:Number(g.v)}))}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name" tick={{fontSize:9}}/><YAxis tick={{fontSize:9}} unit={gr.unidade?" "+gr.unidade:""}/><Tooltip formatter={v=>[v+(gr.unidade?" "+gr.unidade:""),"Valor"]}/><Bar dataKey="val" fill={gr.cor||p.cor} radius={[3,3,0,0]}/></BarChart>}
                            </ResponsiveContainer>
                          </div>
                        ):null;
                      })}
                ...
                {extras.map((pe,pi)=><div key={pe.id} style={{marginBottom:28}}><h3 style={{color:cor,fontSize:13,borderLeft:"4px solid #2d6a4f",paddingLeft:10,marginBottom:10}}>{ativos.length+pi+1}. {pe.nome.toUpperCase()}</h3><p style={{fontSize:12,color:"#444",lineHeight:1.8}}>{pe.intro}</p></div>)}
        ...
        {aba==="baixar"&&(
          <div>
            <h2 style={{color:HC,marginBottom:8}}>⬇️ Baixar Relatório</h2>
            <p style={{fontSize:12,color:"#888",marginBottom:28}}>Visualize o relatório na aba <strong>📄 Relatório</strong> antes de baixar.</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,maxWidth:560}}>
              <div style={{...CD,textAlign:"center",padding:"40px 24px",border:"2px solid #5a4fcf44"}}>
                <div style={{fontSize:52,marginBottom:14}}>📝</div>
                <div style={{fontSize:16,fontWeight:"bold",color:"#5a4fcf",marginBottom:8}}>Baixar em Word</div>
                <div style={{fontSize:12,color:"#888",lineHeight:1.6,marginBottom:20}}>Arquivo .doc editável no Word ou LibreOffice</div>
                <button onClick={()=>dlWord(mes,ano)} style={{background:"#5a4fcf",color:"#fff",border:"none",borderRadius:10,padding:"13px 0",width:"100%",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:14,fontWeight:"bold"}}>📥 Baixar Word (.doc)</button>
              </div>
              <div style={{...CD,textAlign:"center",padding:"40px 24px",border:"2px solid #b5451b44"}}>
                <div style={{fontSize:52,marginBottom:14}}>📄</div>
                <div style={{fontSize:16,fontWeight:"bold",color:"#b5451b",marginBottom:8}}>Baixar em PDF</div>
                <div style={{fontSize:12,color:"#888",lineHeight:1.6,marginBottom:20}}>Salvar como PDF via diálogo de impressão</div>
                <button onClick={dlPDF} style={{background:"#b5451b",color:"#fff",border:"none",borderRadius:10,padding:"13px 0",width:"100%",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:14,fontWeight:"bold"}}>🖨️ Salvar como PDF</button>
              </div>
            </div>
          </div>
        )}
